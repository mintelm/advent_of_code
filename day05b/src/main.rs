...
fn main() {
    ...
    for (p1, p2) in vent_lines.iter() {
        ...
        // diagonal line
            ...
            // directon: right
            if p1.0 < p2.0 {
                x_range = (p1.0..=p2.0).collect();
            // direction: left
            } else {
                x_range = (p2.0..=p1.0).rev().collect();
            }
            // direction: up
            if p1.1 < p2.1 {
                y_range = (p1.1..=p2.1).collect();
            // direction: down
            } else {
                y_range = (p2.1..=p1.1).rev().collect();
            }

            for it in x_range.iter().zip(y_range.iter()) {
                let (x, y) = (*it.0 as usize, *it.1 as usize);
